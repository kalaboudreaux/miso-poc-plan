...
if section == "Overview & Stakeholders":
    ...
    st.info("""
    **POV Statement:** MISO operates one of the largest energy markets in North America — managing a **$32B+ wholesale electricity market**, 
    ensuring grid reliability for **42 million people across 15+ states**, and making thousands of time-critical decisions daily based on complex, 
    multi-source data. The data infrastructure required to support MISO's next generation of AI/ML-driven market analysis is falling behind — 
    and the current Microsoft Fabric roadmap has a **24-month timeline** that does not align with MISO's urgent business needs.
    
    Snowflake will validate in 30 days that it can deliver the advanced analytics, anomaly detection, and scalable ML capabilities that 
    MISO's data science team needs — running natively on Azure, interoperating with Microsoft investments, and accelerating (not replacing) 
    the Fabric journey.
    """)
    ...
    st.markdown("""
    Based on discovery calls with Ben Boutwell (3/24, 4/2, 4/8, 4/17), MISO faces five critical friction points:
    """)

    st.markdown("### 1. Microsoft Fabric Is Not Moving Fast Enough")
    st.warning("The current IT roadmap for Microsoft Fabric has a projected timeline of 24 months. MISO's data science team has NOT yet received access to data and is actively looking for a faster path forward.")

    st.markdown("### 2. Data Is Unclean, Unorganized, and Un-Groomed")
    st.markdown("""
    ...
    st.markdown("### 3. On-Premise Infrastructure Limits Analytical Scale")
    st.markdown("""
    ...
    st.markdown("### 4. Siloed Data Across Groups Creates Trust & Consistency Issues")
    st.markdown("""
    ...
    st.markdown("### 5. Market Integrity at Risk")
    st.markdown("""
    ...
    st.success("""
    **Enable MISO's data science and analytics teams to run advanced AI/ML workloads — market risk analysis, 
    anomaly detection, and forward planning — within 30 days, at a fraction of the cost and timeline of the 
    current Microsoft Fabric roadmap.**
    """)

    st.markdown("### Key Outcomes Targeted")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("""
    ...
    with col1:
        ...
        - Azure ML Studio (1 model in prod)
        - Power BI
        - Spark Notebooks (PySpark)
        - Oracle (on-prem, migrating off)
        - SQL Server (migrating)
        - Netezza (migrating off by EOY)
        - NAS Data Stores (flat files)
        """)
    with col2:
        st.markdown("**In Evaluation**")
        st.markdown("""
        - Microsoft Fabric (IT roadmap)
        - Databricks (explored, Jeff not enthused)
        - Snowflake (Ben's initiative)
        - dbt (Jeff's interest for transforms)
        - Alation (governance/cataloging)
        """)
    with col3:
        st.markdown("**Key Constraints**")
        st.markdown("""
        - Microsoft-first mandate (exec level)
        - Large MSFT agreement in place
        - FERC/NERC compliance required
        - Data residency: Azure region
        - NERC CIP envelope for critical data
        - Tenant migration (govt to commercial)
        - 9 Synapse workspaces x 4 environments
        """)
    ...
    st.markdown("### Key Discovery Findings (from Ben Boutwell calls)")
    st.markdown("""
    ...
    st.error("""
    **If MISO continues on the Fabric-only path**, the data science team will remain blocked for 18-24 more months, 
    market manipulation detection will rely on manual engineering effort, and MISO will fall behind peer ISOs who are 
    already investing in cloud-native analytics. The cost of waiting: **2,800+ engineering hours/year** in lost productivity, 
    undetected market anomalies threatening a $32B market, and a widening capability gap vs. ERCOT and other ISOs.
    """)

    st.success("""
    **If MISO deploys Snowflake alongside Fabric**, the data science team gets data access in weeks (not years), 
    ML models run at 10x current speed, anomaly detection is automated, and Snowflake's Apache Iceberg interop 
    ensures zero conflict with the Microsoft roadmap. Snowflake **accelerates** Fabric — it doesn't replace it.
    """)

    st.markdown("### Predicted Outcomes (30-Day POC)")
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    st.markdown("### Why This Wins at MISO")
    st.markdown("""
    | Advantage | Detail |
    |-----------|--------|
    ...
    st.markdown("### Primary Success Criteria")
    st.markdown("""
    ...
    with st.expander("Test Case 1: Flat File Ingestion & Automated Cleanup"):
        st.markdown("""
        **Objective:** Demonstrate automated ingestion of MISO's semi-structured flat files (GE, PowerGem, OATI formats)
        
        **Steps:**
        1. Ingest representative sample of flat files from NAS stores into Snowflake
        2. Apply Dynamic Tables for automated deduplication and schema normalization
        3. Demonstrate governance (row-level security, data masking) on ingested data
        
        **Success:** 80% reduction in manual data grooming effort, data queryable within 48 hours of ingestion
        """)

    with st.expander("Test Case 2: Market Risk ML Model at Scale"):
        st.markdown("""
        **Objective:** Run Ben's market risk analysis model on Snowflake with elastic compute
        
        **Steps:**
        1. Port existing model to Snowpark ML
        2. Execute on multi-cluster warehouse with varying compute sizes
        3. Compare execution time vs. on-prem baseline
        4. Run 100+ stochastic scenarios in parallel
        
        **Success:** 10x speed improvement, 100+ scenarios completed in time previously needed for 3-4
        """)

    with st.expander("Test Case 3: Anomaly Detection on Market Transactions"):
        st.markdown("""
        **Objective:** Deploy Cortex AI Anomaly Detection on market transaction and pricing data
        
        **Steps:**
        1. Load historical market transaction data (pricing, volumes, time series)
        2. Configure Cortex AI Anomaly Detection for virtual trading patterns
        3. Run against historical known-anomaly periods for validation
        4. Measure false-positive rate vs. current manual detection
        
        **Success:** 50% reduction in false-positive alerts, detection of known historical anomalies
        """)

    with st.expander("Test Case 4: Iceberg/OneLake Bidirectional Integration"):
        st.markdown("""
        **Objective:** Validate that Snowflake reads/writes to Microsoft OneLake without data duplication
        
        **Steps:**
        1. Configure Snowflake Iceberg integration with MISO's Azure Data Lake Storage
        2. Read existing data from OneLake into Snowflake compute
        3. Write results back to OneLake from Snowflake
        4. Verify data accessible from both Fabric and Snowflake engines
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("### Prerequisites (MISO)")
        st.markdown("""
    ...
    with col2:
        st.markdown("### Prerequisites (Snowflake)")
        st.markdown("""
    ...
    st.markdown("### Week 1: Foundation & Environment Setup")
    st.markdown("""
    ...
    st.markdown("### Week 2: Data Ingestion & Pipeline Proof")
    st.markdown("""
    | Day | Activity | Owner | Deliverable |
    |-----|----------|-------|-------------|
    ...
    st.markdown("### Week 3: ML & Analytics Proof")
    st.markdown("""
    | Day | Activity | Owner | Deliverable |
    |-----|----------|-------|-------------|
    ...
    st.markdown("### Week 4: Demo Preparation & Readout")
    st.markdown("""
    | Day | Activity | Owner | Deliverable |
    |-----|----------|-------|-------------|
    ...
    st.markdown("### 30-Day POC Timeline")
    timeline_data = {
        "Week": ["Week 1", "Week 2", "Week 3", "Week 4"],
        "Phase": ["Foundation", "Data Proof", "ML/AI Proof", "Demo & Readout"],
        "Key Milestone": [
            "Environment live, Iceberg validated",
            "Data ingested, 3x speed proven",
            "ML model 10x faster, anomalies detected",
            "IDEA Group demo, business case ready"
        ],
        "Gate": [
            "Go/No-Go: Data accessible?",
            "Go/No-Go: Ingestion meets criteria?",
            "Go/No-Go: ML results meaningful?",
            "Decision: Proceed to production?"
        ]
    }
    st.dataframe(timeline_data, use_container_width=True, hide_index=True)

    st.markdown("### Post-POC Path to Production")
    st.markdown("""
    ...
    st.markdown("### Mutual Commitments")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("#### MISO Commits To:")
        st.markdown("""
        ...
        """)
    with col2:
        st.markdown("#### Snowflake Commits To:")
        st.markdown("""
    ...
    with col1:
        st.markdown("""
        **MISO:**
        - Ben Boutwell: bboutwell@misoenergy.org
        - Jeff Gough: jgough@misoenergy.org
        """)
    with col2:
        st.markdown("""
        **Snowflake:**
        - Kala Boudreaux: kala.boudreaux@snowflake.com
        - Fred Cohagan: fred.cohagan@snowflake.com
        """)
